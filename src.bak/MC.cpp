...
struct SpinFlipper_ : public SpinFlipper {
  ...
  Spin flipped(Spin s) const { flip(s); return s; };
};
...
struct SpinOperator_ : public SpinFlipper_<n> {
  ...
  virtual void reset() final { _operator = &operators[operator_num=_dist(random_engine)]; }
...
public:
  ...
  virtual void local_update(const bool measure_=true) final __attribute__((hot))
  {
  ...
  virtual void global_update() final __attribute__((hot))
  {
      ...
      do {
        ...
        for(Index i : nearestNeighbors(j))
          if ( !_cluster[i] ) {
            ...
            const Spin flipped_s = _flipper->flipped(s);
    ...
    if (true) {
      const LongFloat r = sqrt(LongFloat(.5));
      static const vector<vector<SpinSum>> flip_vecs_list{{{{1,0,0}}},{{{r,r,0}}},{{{r,0,r}}},{{{-r,0,r}}},{{{-r,r,0}}},{{{0,1,0}}},{{{0,r,r}}},{{{0,-r,r}}},{{{0,0,1}}}};
      vector<SpinSum> flip_vecs=flip_vecs_list[operator_num];
      
      LongFloat Y2=0, Y4=0;
      FOR(c, nClusters) {
        const SpinSum y  = _clusterSums[c];
        LongFloat y2 = 0;
        for (SpinSum v: flip_vecs)
          y2 += sq(v|y);
        y2 *= LongFloat(n)/flip_vecs.size();
        Y2 += y2;
        Y4 += y2*y2;
      }
      const LongFloat
...
int main(const int argc, char *argv[])
{
  ...
  if ( !potential_name.empty() )
  {
    ...
    if ( V_str == "s^4" ) {
      ...
      if      (m==2) Assert(false, m); //spin_flipper = &cos2_flipper;
